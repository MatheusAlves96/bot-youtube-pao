...
class AutoplayLogger:
    ...
    def log_session_start(self, reference_video: dict):
        ...
        self.logger.info("=" * 80)
        self.logger.info(f"🎬 NOVA SESSÃO AUTOPLAY - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def log_api_search(self, results_count: int, quota_used: int):
        ...
        self.logger.debug(f"📡 YouTube API Search retornou {results_count} resultados (quota: +{quota_used})")
    ...
    def log_batch_duration_api(self, video_count: int, elapsed: float, speed: float, quota_saved: int):
    ...
    def log_duration_filter(self, video_title: str, duration: float, reason: str, passed: bool):
        """Registra filtro de duração"""
        emoji = "✅" if passed else "⏭️"
        status = "APROVADO" if passed else "REJEITADO"
        self.logger.debug(f"{emoji} [{status}] {video_title[:60]} | {duration}min | {reason}")

    def log_filter_summary(self, approved: int, rejected: int, min_duration: int, max_duration: int):
    ...
    def log_ai_validation_result(self, video_title: str, approved: bool, reason: str, confidence: float):
    ...
    def log_queue_added(self, video_title: str, queue_position: int):
        ...
        self.logger.info(f"✅ Adicionado à fila (posição #{queue_position}): {video_title}")
    ...
    def log_session_end(self, success: bool, videos_added: int, total_time: float):
        ...
        self.logger.info(f"📊 Vídeos adicionados: {videos_added} | Tempo total: {total_time:.2f}s")
        self.logger.info("=" * 80 + "\n")
    ...
    def log_error(self, error_msg: str, exception: Exception = None):
        ...
        if exception:
            self.logger.error(f"   └─ Exceção: {type(exception).__name__}: {str(exception)}")
